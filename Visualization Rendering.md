...
An example is the navigation mesh visualization.
...
There is an [[Actor]], `ARecastNavmesh`, that has a `UPrimitiveComponent* RenderingComp`.
`RenderingComp` is initialized with `NewObject<UNavMeshRenderingComponent>` in `PostInitProperties`, `PostLoad`, `RerunConstructionScripts`, `OnRegistered`, and `PostEditUndo`.
...
The nav mesh has `FNavMeshSceneProxy`.
It implements `GetDynamicMeshElements` to generate lines and triangles.
...
It also has a companion class named `FNavMeshSceneProxy` inheriting from `FDebugRenderSceneProxy`.
It also has a companion class named `FNavMeshSceneProxyData`.
...
That marks the end of `GatherData`.
...
I assuemt his adds stuff to `FDebugMeshData::Indices`.
